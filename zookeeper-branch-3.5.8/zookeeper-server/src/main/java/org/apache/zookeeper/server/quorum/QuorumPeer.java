...
public class QuorumPeer extends ZooKeeperThread implements QuorumStats.Provider {
    ...
    public synchronized void start() {
        ...
        try {
            // # 启动Jetty服务，默认8080端口，用来查看服务器状态信息
            adminServer.start();
        ...
        // ! 初始化集群选举leader相关对象数据
        startLeaderElection();
        // ! 启动集群选举leader线程,需要跳转到 QuorumPeer#run
        super.start();
    }
                ...
                case LOOKING: // ! 选举状态
                    ...
                    if (Boolean.getBoolean("readonlymode.enabled")) {
                        ...
                        try {
                            ...
                            // ! lookForLeader()需要进入快速选举算法 FastLeaderElection#lookForLeader()
                    ...
                    break;
                case OBSERVING: // * observing 状态 不用关注
                    try {
                        LOG.info("OBSERVING");
                    ...
                    break;
                case FOLLOWING: // ! following 状态
                    try {
                       LOG.info("FOLLOWING");
                        // # makeFollower创建一个follower节点, FollowerZooKeeperServer
                        setFollower(makeFollower(logFactory));
                        // ! 与leader建立连接，并且接收leader的数据同步
                        follower.followLeader();
                    } catch (Exception e) {
                       LOG.warn("Unexpected exception",e);
                    } finally {
                       // # 如果leader挂了会触发异常，执行finally
                       follower.shutdown();
                       setFollower(null);
                       // ! 更新自己的状态为looking，进行下一轮while循环选举
                       updateServerState();
                    }
                    break;
                case LEADING: // ! leading状态
                    LOG.info("LEADING");
                    try {
                        // # makeLeader创建并设置leader， LeaderZooKeeperServer
